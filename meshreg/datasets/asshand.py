...
class AssHand(Ass101):
    ...
    def load_dataset(self,verbose=False):
        ...
        try:
            self.env_midpe=lmdb.open(os.path.join(f'../lmdbsv2/asshand_midpe_world16x1@30fps_2sets_pdobsv',self.split),readonly=True,lock=False,readahead=False,meminit=False,\
                                map_size=(1024)**3,max_spare_txns=32,max_dbs=1000)
        except:
            self.env_midpe=None
        #compute window_info
